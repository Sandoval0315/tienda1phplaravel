...
<div class="container mx-auto px-4 py-12">
    ...
    <div class="flex flex-wrap justify-between items-center mb-12 pb-4 border-b">
        ...
        <div class="relative">
            <select id="sort-select"
                class="border border-gray-200 rounded-lg px-4 py-2 text-sm bg-white cursor-pointer">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8" id="products-grid">
        ...
        <div class="product-card group" data-category="{{ $product->category }}" data-price="{{ $product->price }}"
            ...
            <div class="relative overflow-hidden bg-gray-100 rounded-2xl aspect-[3/4] cursor-pointer"
                ...
                @if($loop->first)
                <div class="absolute top-4 left-4 bg-black text-white text-xs px-3 py-1 rounded-full z-10">
                    Nuevo
                </div>
                @endif

                <!-- Botón rápido de añadir al carrito (AHORA SÍ DENTRO DEL MISMO CONTENEDOR) -->
                <button onclick="event.stopPropagation(); addToCart({{ $product->id }})"
                    class="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-white text-black px-6 py-2 rounded-full text-sm font-medium opacity-0 group-hover:opacity-100 transition duration-300 hover:bg-black hover:text-white z-10">
                    <i class="fa-solid fa-bag-shopping mr-2"></i>
                    Añadir
                </button>
            </div>

            <!-- Información del producto (fuera del contenedor de imagen) -->
